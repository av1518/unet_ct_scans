...
class SimpleUNet(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv1 = self.conv_block(
            in_channels, out_channels=16, kernel_size=3, stride=1, padding=1
        )
        self.maxpool1 = self.maxpool_block(kernel_size=2, stride=2, padding=0)
        ...
        self.final = self.final_layer(16, out_channels, 1, 1, 0)
    ...
    def transposed_block(  # inserting zeros between the input values (up-sampling) and then applying a convolution.
    ...
    def final_layer(self, in_channels, out_channels, kernel_size, stride, padding):
        """
        Define separately so that we can change the final layer parameters on its own.
        """
        final = nn.Conv2d(
            in_channels=in_channels,
            out_channels=out_channels,
